...
const ListOfBloggers = () => {
    ...
    const getUsers = async () => {
        ...
        setBloggers(users.data);
      };
    const deleteBlogger = async(userId)=>{
        try{
        ...
        {!loading ? (
          ...
          .filter(
                  (item) =>
                    ...
                    item.isBlogAdmin &&
                    item.isBlogAdmin.toString() === "true"
                )
            .map((user, i) => (
              <tr key={i}>
                <td>{i + 1}</td>
                ...
                <td>
                <select
                                value={isBlogAdmin[user._id] || (user.isBlogAdmin ? "true" : "false")}
                                onChange={(e) => setIsBlogAdmin({ ...isBlogAdmin, [user._id]: e.target.value })}
                                disabled={!openEdit}
                            >
                        ...
                        <div id="del-post-wrapper">
                          ...
                          <div id="del-post-wrapper-sec-two">
                            <span style={{ color: "#0F4157" }}>
                              Are you sure you want to delete this Blogger ({`${blogger.userName}`})?
                            </span>
                            <button
                              id="del-post-wrapper-sec-two-btn1"
                              onClick={() => {
                                 deleteBlogger(blogger._id);
                              }}
                            >
                              Yes
                            </button>
