  ...
  const fetchAverageRating = async (productId) => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/product-rating/average-rating/${productId}`
      );
      ...
      return null; // or some default rating value
    }
  };
    ...
    try {
      ...
      const productsWithRatings = await Promise.all(
        data.map(async (product) => {
          const avgRating = await fetchAverageRating(product._id);
          console.log("avgRating : ", avgRating);
          return { ...product, avgRating }; // Append average rating to product
        })
      );

      setProducts(productsWithRatings);
      setGroupedProducts(groupByCategory(productsWithRatings));
      setTemp(data);
