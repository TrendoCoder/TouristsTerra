...
const BookingHistory = () => {
  ...
  const fetchBookingHistory = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/booking-history/get-booking-history/${user._id}`
      );
  ...
  const deleteBooking = async (bookingId) => {
    try {
      await axios.delete(
        `http://localhost:3001/api/booking-history/delete-booking/${bookingId}`
      );
      fetchBookingHistory();
    } catch (error) {
            ...
            {bookingHistory.map((booking, index) => (
              ...
              >
                ...
                <div className="flex-none ml-4 space-y-2">
                  <button
                    onClick={() => toggleModal(booking.guideId._id)}
