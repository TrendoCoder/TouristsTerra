...
const ReserveAccomodation = ({ setOpen, hotelId }) => {
  ...
  const handleClick = async () =>{
    ...
    //         const res = axios.put("http://localhost:3001/api/rooms/availability/"); 
    //     })
    // }catch(err){
    ...
    return (
      <div id="acc-reserve">
        <div id="acc-reserve-container">
          <i class="fa-solid fa-circle-xmark" onClick={() => setOpen(false)}></i>
          ...
          <button onClick={handleClick} id="r-btn"> Resrve Now</button>
        </div>
      </div>
    );
  }
